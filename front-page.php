...
<div id="content" class="site-content">
    ...
    if ( !newsx_get_option('front_page_hide_blog_feed') ) {
        ...
        if ( 'posts' === get_option('show_on_front') ) {
            ...
            // Initialize array to store excluded post IDs
            ...
            echo '<div class="newsx-row newsx-fp-row-extra">';
                echo '<div class="newsx-row-inner newsx-flex-nowrap newsx-content-nosidebar'. esc_attr($wrapper_class) .'">';
                    ...
                    if ( $magazine_posts->have_posts() ) :
			            ...
			            // Collect post IDs for exclusion in second loop
...
*/
			...
			// Definisci la query per ottenere solo i post della categoria 'news'
			// In testa al file del template:
			$paged = 1;
			if ( get_query_var('paged') ) {
				$paged = get_query_var('paged');
			} elseif ( get_query_var('page') ) {
				// Se stai usando un Page Template
				$paged = get_query_var('page');
			} 
			
			$args = array(
				'category_name' => 'news,recensione,prova', // Usa lo slug della categoria
				'post_type' => 'post',
				'posts_per_page' => get_option('posts_per_page'), // Numero di post per pagina
				'paged' => $paged, // Supporta la paginazione
				'post__not_in' => $excluded_post_ids // Exclude posts from first loop
			);

				// Esegui la query personalizzata per caricare solo articoli dentro la categoria "news"
				$news_query = new WP_Query( $args );
				// Salva il main query e sostituiscilo temporaneamente
				global $wp_query;
				...
				if ( $news_query->have_posts() ) :
					...
					while ( $news_query->have_posts() ) :
						...
						// Il tuo template dei post
						$posts_layout = new \Newsx_Blog_Posts_Presets();
						$posts_layout->display();

					endwhile;

					// Qui la pagination legge $wp_query (ovvero $news_query)
					get_template_part( 'template-parts/blog-page/pagination' );

					// Ripristina il main query
